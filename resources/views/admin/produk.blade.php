<x-dashboard>
    ...
    <!-- Page Content -->
    <div x-data="{ search: '', filterStatus: 'Semua Status', showToast: false, toastMessage: '' }" class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <button @click="toastMessage = 'Membuka form produk baru...'; showToast = true; setTimeout(() => showToast = false, 3000)" class="px-5 py-2.5 bg-[#0A2540] hover:bg-[#0d2e59] text-white font-bold rounded-xl transition-colors shadow-sm inline-flex items-center gap-2">
        ...
        <div class="flex flex-col sm:flex-row gap-4 mb-6">
            <div class="relative flex-1">
                ...
                <input type="text" x-model="search" class="bg-white border border-gray-200 text-gray-900 text-sm rounded-xl focus:ring-[#00E5FF] focus:border-[#00E5FF] block w-full pl-10 p-3" placeholder="Cari produk...">
            </div>
            <select x-model="filterStatus" class="bg-white border border-gray-200 text-gray-700 text-sm rounded-xl focus:ring-[#00E5FF] focus:border-[#00E5FF] p-3 min-w-[150px]">
        ...
        <div class="space-y-4">
            ...
            <div x-show="(filterStatus === 'Semua Status' || filterStatus === '{{ $product['status'] }}') && '{{ strtolower($product['name']) }}'.includes(search.toLowerCase())" class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden flex flex-col md:flex-row group hover:shadow-md transition-shadow">
                ...
                <div class="p-6 flex-1 flex flex-col justify-between">
                    ...
                    <div class="flex items-start justify-between gap-4 mb-4">
                        ...
                        <div class="flex items-center flex-shrink-0" title="Aktifkan/Nonaktifkan Manual">
                            <label class="relative inline-flex items-center cursor-pointer">
                                <input type="checkbox" @change="toastMessage = 'Status {{ $product['name'] }} diubah'; showToast = true; setTimeout(() => showToast = false, 3000)" value="" class="sr-only peer" {{ $product['status'] == 'Aktif' ? 'checked' : '' }}>
                    ...
                    <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 mt-4">
                        ...
                        <div class="flex items-center gap-2 flex-shrink-0">
                            <button @click="toastMessage = 'Membuka form edit untuk {{ $product['name'] }}'; showToast = true; setTimeout(() => showToast = false, 3000)" class="px-4 py-2 bg-white border border-gray-200 text-gray-700 hover:bg-gray-50 rounded-xl text-sm font-medium transition-colors flex items-center gap-2">
                                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                Edit
                            </button>
                            <button @click="toastMessage = 'Produk {{ $product['name'] }} berhasil dihapus'; showToast = true; setTimeout(() => showToast = false, 3000)" class="p-2 bg-white border border-red-100 text-red-500 hover:bg-red-50 rounded-xl transition-colors" title="Hapus Produk">
