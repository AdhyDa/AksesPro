        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-2 overflow-x-auto pb-2 sm:pb-0 hide-scrollbar">
                <button @click="category = 'Semua'"
                    :class="category === 'Semua' ? 'bg-[#0A2540] text-white border-[#0A2540]' :
                        'bg-white border-gray-200 text-gray-600 hover:bg-gray-50'"
                    class="px-4 py-2 border rounded-full text-sm font-medium whitespace-nowrap transition-colors">Semua</button>
                <button @click="category = 'Streaming'"
                    :class="category === 'Streaming' ? 'bg-[#0A2540] text-white border-[#0A2540]' :
                        'bg-white border-gray-200 text-gray-600 hover:bg-gray-50'"
                    class="px-4 py-2 border rounded-full text-sm font-medium whitespace-nowrap transition-colors">Streaming</button>
                <button @click="category = 'Musik'"
                    :class="category === 'Musik' ? 'bg-[#0A2540] text-white border-[#0A2540]' :
                        'bg-white border-gray-200 text-gray-600 hover:bg-gray-50'"
                    class="px-4 py-2 border rounded-full text-sm font-medium whitespace-nowrap transition-colors">Musik</button>
                <button @click="category = 'Desain'"
                    :class="category === 'Desain' ? 'bg-[#0A2540] text-white border-[#0A2540]' :
                        'bg-white border-gray-200 text-gray-600 hover:bg-gray-50'"
                    class="px-4 py-2 border rounded-full text-sm font-medium whitespace-nowrap transition-colors">Desain</button>
                <button @click="category = 'Produktivitas'"
                    :class="category === 'Produktivitas' ? 'bg-[#0A2540] text-white border-[#0A2540]' :
                        'bg-white border-gray-200 text-gray-600 hover:bg-gray-50'"
                    class="px-4 py-2 border rounded-full text-sm font-medium whitespace-nowrap transition-colors">Produktivitas</button>
            </div>
        </div>
            ...
            @foreach ($products as $product)
                <div x-show="category === 'Semua' || category === '{{ $product->category }}'"
                    ...
                    <div class="pt-12 p-6 flex-1 flex flex-col">
                        ...
                        <p class="text-sm text-gray-500 mb-4 line-clamp-2">Akses fitur premium tanpa batas, nikmati
                            layanan terbaik dengan harga super hemat.</p>
                        ...
                        <div class="mt-auto pt-4 border-t border-gray-50 flex items-end justify-between">
                            ...
                            <a href="{{ route('user.katalog.detail', $product->slug) }}"
                                class="bg-[#0A2540] hover:bg-[#0d2e59] text-white px-5 py-2.5 rounded-xl font-semibold text-sm transition-colors shadow-sm group-hover:shadow-md text-center">
                                Beli
                            </a>
                        </div>
                    </div>
