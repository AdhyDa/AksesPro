    ...
    <div class="space-y-6">
        ...
        <div>
            <a href="{{ route('user.katalog') }}"
                class="inline-flex items-center text-sm font-medium text-gray-500 hover:text-[#0A2540] transition-colors bg-white px-3 py-1.5 rounded-lg border border-gray-200 shadow-sm">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <!-- Header Gradient Box -->
                <div
                    class="bg-gradient-to-br from-[#0A2540] to-[#163b63] rounded-2xl p-6 sm:p-8 text-white relative overflow-hidden shadow-sm">
                    <!-- Decorative Elements -->
                    <div
                        class="absolute top-0 right-0 -mr-12 -mt-12 w-40 h-40 rounded-full bg-[#00E5FF] opacity-10 blur-2xl">
                    </div>
                    <div class="absolute bottom-0 left-0 -ml-8 -mb-8 w-24 h-24 rounded-full bg-white opacity-5 blur-xl">
                    </div>
                    ...
                    <div class="relative z-10 flex flex-col md:flex-row items-start md:items-center gap-6">
                        ...
                        <div>
                            ...
                            <div class="flex flex-wrap items-center gap-3">
                                <span
                                    class="bg-[#00E5FF]/20 text-[#00E5FF] font-semibold px-3 py-1 rounded-md text-sm">{{ $product->category }}</span>
                ...
                <div class="bg-white rounded-2xl shadow-xl shadow-[#0A2540]/5 border border-gray-100 p-6 sticky top-6">
                    ...
                    <div class="space-y-4 mb-6">
                        ...
                        <div class="flex justify-between items-center text-sm">
                            ...
                            @if ($product->stock > 0)
                                <span
                                    class="font-semibold text-emerald-600 bg-emerald-50 px-2.5 py-1 rounded-md text-xs">Tersedia
                                    ({{ $product->stock }})</span>
                            @else
                                <span
                                    class="font-semibold text-red-600 bg-red-50 px-2.5 py-1 rounded-md text-xs">Habis</span>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="bg-gray-50 rounded-xl p-5 mb-6 border border-gray-100">
                        ...
                        <div class="flex flex-col sm:flex-row sm:items-end justify-between gap-1">
                            <div>
                                <p class="text-3xl font-black text-[#0A2540]">Rp
                            ...
                            @if ($product->original_price)
                                <div class="text-right">
                                    ...
                                    @php
                                        $discount = round(
                                            (($product->original_price - $product->aksespro_price) /
                                                $product->original_price) *
                                                100,
                                        );
                                    @endphp
                                    <span class="text-xs font-bold text-red-500 block">Hemat
                                        {{ $discount }}%</span>
                                </div>
                            @endif
                        </div>
                    ...
                    <form action="{{ route('user.poin.redeem', $product->id) }}" method="POST" class="w-full">
                        @csrf
                        <button type="submit"
                            class="w-full bg-[#0A2540] hover:bg-[#0d2e59] text-white py-3.5 rounded-xl font-bold text-lg transition-all transform hover:scale-[1.02] shadow-md flex items-center justify-center gap-2 {{ $product->stock <= 0 || $user['points'] < $product->aksespro_price ? 'opacity-50 cursor-not-allowed hover:scale-100' : '' }}"
                            {{ $product->stock <= 0 || $user['points'] < $product->aksespro_price ? 'disabled' : '' }}>
